...
def test_imp_features_only(imp_features_only_predictions):
    ...
    assert f1 >= 0.70, f"F1 too low with only important features: {f1:.3f}"

    confs = avg_class_conf(probs, probs.argmax(axis=1))
    for c in confs:
        if confs[c] is not None:
            assert confs[c] >= 0.65, f"Confidence too low for class {c}: {confs[c]:.3f}"
...
def test_no_imp_features(no_imp_features_predictions):
    ...
    f1 = f1_score(y_true, y_pred, average="macro")
    assert f1 >= 0.55, f"F1 too low with no important features: {f1:.3f}"

    confs = avg_class_conf(probs, probs.argmax(axis=1))
    for c in confs:
        if confs[c] is not None:
            assert confs[c] >= 0.50, f"Confidence too low for class {c}: {confs[c]:.3f}"
...
def test_noisy_non_imp(noisy_non_imp_predictions):
    ...
    f1 = f1_score(y_true, y_pred, average="macro")
    assert f1 >= 0.60, f"F1 too low with noise in non-important features: {f1:.3f}"
...
def test_noisy_imp_features(noisy_imp_features_predictions):
    ...
    f1 = f1_score(y_true, y_pred, average="macro")
    assert f1 <= 0.50, f"F1 unexpectedly high with noise in important features: {f1:.3f}"

    confs = avg_class_conf(probs, probs.argmax(axis=1))
    for c in confs:
        if confs[c] is not None:
            assert confs[c] <= 0.50, f"Confidence too high for class {c} under noisy-imp: {confs[c]:.3f}"
...
def test_random_noise(random_noise_predictions):
    ...
    f1 = f1_score(y_true, y_pred, average="macro")
    assert f1 <= 0.30, f"F1 unexpectedly high on random noise: {f1:.3f}"
